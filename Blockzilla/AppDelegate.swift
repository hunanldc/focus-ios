...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, open url: URL, sourceApplication: String?, annotation: Any) -> Bool {
        ...
        guard let scheme = components.scheme, let host = url.host, urlSchemes.contains(scheme) else {
            return false
        }
        ...
        switch host {
        case "open-url":
            let url = unescape(string: query["url"]) ?? ""
            if application.applicationState == .active {
                ...
                browserViewController.submit(url: URL(string: url)!)
            } else {
                queuedUrl = URL(string: url)!
            }
            return true
        default: ()
        }
